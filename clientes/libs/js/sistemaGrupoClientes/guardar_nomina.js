...
function cargar_nomina_cliente(id_nomina, id_empresa) {
	...
	ajax.open("POST", "cargar_nomina.php", true);
...
function guardar_empleados_nomina(id_empresa, id_nomina) {
	...
	div_mensaje = document.getElementById('alerta_nomina');
	div_mensaje.innerHTML = 'Nomina Guardada Correctamenteeeee...';
	alert("Nomina "+id_nomina+" Guardada Correctamenteeeee.")
...
function generar_reporte_nomina(id_empresa, id_nomina) {
	ajax = objetoAjax();

	ajax.open("POST", "imprimir_prenomina.php", true);

	div_mensaje.innerHTML = 'Generando Reporte de Nomina...';
...
function autorizar_nomina(id_nomina) {
	...
	if(confirm("Al autorizar la nomina se procedera a la dispersi�n y generaci�n de recibos. �Desea continuar?")) {
		...
		ajax.open("POST", "autorizar_nomina.php", true);
...
function consultar_reporte() {
	...
	if(tipo == "empleados") {
		pagina = "reporte_empleados.php";
	} else if(tipo == "nominas") {
		pagina = "reporte_nomina_consulta.php";
	...
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
			...
			//obtengo el listado de nominas
			reporte.innerHTML = ajax.responseText
