...
function eliminar_publicacion(id_documento){
	...
	ajax.open("POST", "publicacion_eliminar.php",true);
  ...
  ajax.onreadystatechange=function() {
	if (ajax.readyState==4) {		
		...
		cargarPagina('publicaciones.php', 'mainContent');
...
function eliminar_usuarios_clientes(id_usuario, id_empresa, usuario){
	...
	ajax.open("POST", "usuario_empresas_eliminar.php",true);
...
function eliminar_empresa(id_empresa){
   ...
   if ( eliminar ) {
	   ...
	   ajax.open("POST", "empresa_procesos.php", true);
	   ajax.onreadystatechange=function() {
		   if (ajax.readyState==4) {
		   //mostrar resultados en esta capa
