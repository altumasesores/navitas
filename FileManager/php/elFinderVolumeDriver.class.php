...
abstract class elFinderVolumeDriver {
	...
	public function mount(array $opts) {
		//debe recibir algo asi 
		/*array(
		    /////-----------'defaults'   => array('read' => true, 'write' => false),
			'driver'        => 'LocalFileSystem',   // driver for accessing file system (REQUIRED)
			'path'          => '../../documentos/',         // path to files (REQUIRED)
			'URL'           => dirname($_SERVER['PHP_SELF']) . '/../../documentos/', // URL to files (REQUIRED)
			'accessControl' => 'access'             // disable and hide dot starting files (OPTIONAL)			
		)*/
		if (!isset($opts['path']) || $opts['path'] === '') {//si path es vacio regresa false
			return false;
		}
		
		$this->options = array_merge($this->options, $opts);//se combina las opciones especificadas con las opciones default de clase.
		...
		//si hay valores de lectura y escritura para el directorio general
		$this->defaults = array(
		...
		if (!empty($this->options['accessControl'])) {//si hay un modo de control de acceso al directorio
	...
	public function mkdir($dst, $name) {
		...
		if ($this->commandDisabled('mkdir')) {
			//echo $this->setError(elFinder::ERROR_PERM_DENIED);
