...

{
	...
	public function EmpleadosNomina($id_nomina,$tipoNomina,$tipo_nom,$sueldo_integrado,$dias_trabajados)

        {
			...
			//ID Y "SEMANAL"
				...
				foreach($consulta2 as $Row2){
					...
					$SueldoDiario=$Row2['sueldo_diario_emp'];

					$SueldoDiarioImss= $Row2['sueldo_diario_imss'];					
					...
					$TotalSueldoImss=$this->redondear($SalarioImss*$DiasTrabajados);

					

					$SueldoparaIMSS=$this->redondear($SueldoDiarioImss*$DiasTrabajados);
					...
					$IMSS=$this->getIMSS($SueldoparaIMSS);
					...
					$IMSS_Imss=$this->getIMSS($SueldoparaIMSS);
					...
					$consulta3 = $this->db->prepare("UPDATE deducciones_empleado SET ISR='$ISR', IMSS='$IMSS' WHERE id_nomina= $id_nomina AND id_empleado=$id_empleado ");
					...
					$consulta4 = $this->db->prepare("UPDATE percepciones_empleado SET subsidio_empleo='$Subsidio' WHERE id_nomina= $id_nomina AND id_empleado=$id_empleado ");

					$consulta4->execute();

					*/	//NO DEBE EXISTIR ISR NI IMSS NI SUBSIDIO EN LA TABLA DE NOMINAS NORMALES
		...
		/* public function calcular_ISR($SueldoDiario,$tipoNomina)

        {

			

				$resultado=$SueldoDiario*2;
...
public function getIMSS($sueldo){
	...
	$this->resultado_factor_integracion = $sueldo * $this->factor_integracion;

	

	$this->IMSS = $this->resultado_factor_integracion * ($this->cuota_trabajador
