<?php 
	...
	$objPHPExcel->getActiveSheet(0)->SetCellValue('A3', "NOMINA: $id_nomina    $tipo_nomina");
...
//	$AIS= $row_empleados['dias_trabajados'] * $sueldo_imss;

	$AIS= redondear( $row_empleados['total_ReciboNomina']);
	...
	$objPHPExcel->getActiveSheet(0)->SetCellValue('C'.$cont, "$AIS");
//	$objPHPExcel->getActiveSheet()->SetCellValue('C'.$cont, "072");
//	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$cont, "01");
//	$objPHPExcel->getActiveSheet()->SetCellValue('E'.$cont, "$cuenta");
	...
	$total_percepciones_empleado2= (float)$row_empleados2['total_sueldo'] + (float)$row_empleados2['total_horas_extras'] + (float)$row_empleados2['total_domingos_trabajados'] + (float)$row_empleados2['total_turnos_trabajados'] + (float)$row_empleados2['total_descanso_trabajado'] + (float)$row_empleados2['total_dias_festivos'] + (float)$row_empleados2['total_vacaciones'] + (float)$row_empleados2['complemento_sueldo'] + (float)$row_empleados2['otras_percepciones'];
	
	$total_deducciones_empleado2=  (float)$row_empleados2['prestamos'] + (float)$row_empleados2['caja_ahorro'] + (float)$row_empleados2['fonacot'] + (float)$row_empleados2['infonavit'] +  (float)$row_empleados2['otras_deducciones'] + (float)$row_empleados2['prestamo_ascon'] ;
	
	$percepcion_emp2= $total_percepciones_empleado2 - $total_deducciones_empleado2;
	
	
	
	$sueldo_imss2=73;

	//$AIS2= $row_empleados2['dias_trabajados'] * $sueldo_imss2;
	
	$Total_Sueldo=redondear( $percepcion_emp2 - $row_empleados2['total_ReciboNomina'] ) ; 
	...
	$objPHPExcel->getActiveSheet(1)->SetCellValue('C'.$cont2, "$Total_Sueldo");
